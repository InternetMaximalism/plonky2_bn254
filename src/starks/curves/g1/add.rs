...
/// The caller must ensure that a and b are not points at infinity.
/// If b = -a, a proof cannot be generated.
/// This is to ensure that subsequent calls to this function are safe.
pub(crate) fn generate_g1_add<F: RichField>(a: G1<F>, b: G1<F>) -> (G1<F>, G1AddAux<F>) {
    ...
    let (lambda, lambda_i64, lambda_aux) = if !is_x_eq.is_one() {
        ...
        // If a.x == b.x, then a must be equal to b, otherwise infinity point is
        // returned, which will cause an error in subsequent calculations.
