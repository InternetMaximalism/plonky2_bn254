...
impl<T: Copy + Clone + Default> G1ScalarMulView<T> {
    pub(super) fn to_slice(&self) -> &[T] {
        debug_assert_eq!(
            ...
            G1_SCALAR_MUL_VIEW_LEN * std::mem::size_of::<T>()
        );
        unsafe {
...
mod tests {
    ...
    use crate::starks::curves::g1::scalar_mul_view::{
        FREQ_COL, INPUT_FILTER_COL, OUTPUT_FILTER_COL, RANGE_COUNTER_COL, TIMESTAMP_COL,
    };
