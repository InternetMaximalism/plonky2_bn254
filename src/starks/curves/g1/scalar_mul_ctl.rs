...
pub(crate) fn g1_scalar_mul_ctl<F: Field>() -> Vec<CrossTableLookup<F>> {
...
pub(crate) fn g1_generate_ctl_values<F: RichField>(
    ...
    for (input, timestamp) in inputs {
        ...
        input_ctl.extend_from_slice(G1::from(input.x).to_slice());
        input_ctl.extend_from_slice(G1::from(input.offset).to_slice());
        ...
        output_ctl.extend_from_slice(G1::from(output).to_slice());
